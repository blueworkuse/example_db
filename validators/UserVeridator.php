<?php
    ...
    class UserVeridator {
        ...
        /**
         * 驗證此帳號 memberID 跟 開通碼 hash 是否已存在於資料庫中
         */
        public function isReady2Active($memberID, $active){
            $result = Database::get()->execute('SELECT username FROM members WHERE memberID = :memberID AND active = :active', array(':memberID' => $memberID, ':active' => $active));
            if(isset($result[0]['username']) and !empty($result[0]['username'])){
                return true;
            }else{
              $this->error[] = '提供的用戶名已被使用.';
